...
public class UserGet {
    public ArrayList<User> getListUser() throws SQLException {
        ...
        String sql = "SELECT * FROM musicshop.users";
    ...
    Connection connection = DBConnect.getConnecttion();
    String sql = "SELECT * FROM musicshop.users WHERE user_name = '" + name + "'";
    ...
    public boolean insertUser(User u) {
        Connection connection = DBConnect.getConnecttion();
        String sql = "INSERT INTO musicshop.users VALUES(?,?,?,?,?,?)";
        try {
            PreparedStatement ps = connection.prepareCall(sql);
     ...
     public User login(String name, String password) {
        ...
        String sql = "select * from musicshop.users where user_name='" + name + "' and user_pass='" + password + "'";
        PreparedStatement ps;
        try {
     ...
     Connection connection = DBConnect.getConnecttion();
     String sql = "SELECT * FROM musicshop.users WHERE user_id = '" + userID + "'";
     PreparedStatement ps = connection.prepareCall(sql);
     ResultSet rs = ps.executeQuery();
     User user = new User();
     while (rs.next()) {
     ...
     public boolean updateUser(User u) {
       ...
       // String sql = "UPDATE product SET product_id = ?, category_id = ?, product_name = ?, product_image = ?, product_image_forward = ?, product_image_back = ?, product_price = ?, product_description = ? WHERE product_id = ?";
        String sql = "UPDATE musicshop.users SET user_id=?, user_name=?, user_email=?, user_pass=?, user_role=?, user_phone=? WHERE user_id = ?";
